// Legacy "Suivi de conversion" page — superseded by /dashboard/suivi.
// Kept intact (colocated, non-routed) for future reuse. Not imported anywhere.
...
export default async function LegacyTrackingContent() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        {[
          { icon: TrendingUp, label: 'Uplift conversion', value: uplift > 0 ? `+${uplift.toFixed(1)}%` : `${uplift.toFixed(1)}%`, color: uplift >= 0 ? 'text-success' : 'text-danger' },
          { icon: Euro, label: 'Revenus récupérés', value: `€${recoveredRevenue.toLocaleString('fr-FR')}`, color: 'text-primary' },
          { icon: BarChart3, label: 'Taux moyen (avant)', value: `${(avgBefore * 100).toFixed(2)}%`, color: 'text-text-primary' },
          { icon: Target, label: 'Taux moyen (après)', value: `${(avgAfter * 100).toFixed(2)}%`, color: 'text-text-primary' },
        ].map((stat) => (
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 mb-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-syne font-semibold text-text-primary">Taux de conversion — 30 derniers jours</h2>
          {firstFixDate && (
      ...
      {typedFixes.length > 0 && (
        <div className="bg-surface border border-border rounded-2xl p-6">
          <h2 className="font-syne font-semibold text-text-primary mb-4">Correctifs contribuant à l&apos;uplift</h2>
          <div className="space-y-2">
            {typedFixes.map((fix) => (
              <div key={fix.id} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                <div>
                  <p className="text-text-primary text-sm font-medium">{fix.title}</p>
                  <p className="text-text-muted text-xs mt-0.5">Appliqué le {new Date(fix.created_at).toLocaleDateString('fr-FR')}</p>
                </div>
                <span className="text-success text-sm font-semibold">+€{fix.impact_euros}/mois</span>
              </div>
            ))}
          </div>
          <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
            <span className="text-text-secondary text-sm font-medium">Total récupéré</span>
            <span className="font-syne font-bold text-xl text-primary">€{recoveredRevenue.toLocaleString('fr-FR')}/mois</span>
          </div>
        </div>
      )}
    </div>
