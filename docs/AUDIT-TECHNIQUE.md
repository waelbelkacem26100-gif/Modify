...
| 3 | **`CRON_SECRET` absente** — les crons ne sont pas sécurisés ; fallback dangereux sur `SHOPIFY_CLIENT_SECRET` dans `lib/approval-token.ts` | 🔴 | À ajouter dans Vercel |
...
### Bug 🔴 — Fallback dangereux dans `lib/approval-token.ts`
```typescript
function key(): string | null {
  return process.env.CRON_SECRET || process.env.SHOPIFY_CLIENT_SECRET || null
}
```
Si `CRON_SECRET` absent, les tokens d'approbation sont signés avec `SHOPIFY_CLIENT_SECRET` — clé partagée avec Shopify OAuth. **Corriger en ajoutant `CRON_SECRET` dans Vercel.**
...
|-----|----------|---------|-------------|
| Fallback `SHOPIFY_CLIENT_SECRET` pour signing | 🔴 | `lib/approval-token.ts` | Partage de clé entre deux systèmes |
...
## Actions Restantes (hors périmètre de ce commit)
...
2. **Remplacer `ESTIMATED_DAILY_SESSIONS`** par une vraie intégration analytics Shopify.

3. **Incrémenter `views_count`** dans le handler `products/create` ou `products/update`.
